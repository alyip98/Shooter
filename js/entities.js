class Entity {
  ...
  tick(dt) {
    ...
    if (Math.abs(this.v) < 0.1) this.v = 0;
    ...
    if ((collision = this.checkWallCollisions())) {
      ...
      if (collision.friction) this.v *= 1 - chg.friction;
    } else {
      this.x += vx * dts;
      this.y += vy * dts;
    }
  ...
  physicsCollision(other) {
    ...
    var cr = -(
      this.getCoords().distTo(other.getCoords()) -
      this.size -
      other.size
    );
    var temp1 =
    ...
    var temp2 =
      ...
      (m1 + m2);
    var vx1 =
      temp1 * Math.cos(phi) + v1 * Math.sin(theta1 - phi) * Math.sin(phi);
    var vy1 =
      temp1 * Math.sin(phi) + v1 * Math.sin(theta1 - phi) * Math.cos(phi);

    var vx2 =
      temp2 * Math.cos(phi) + v2 * Math.sin(theta2 - phi) * Math.sin(phi);
    var vy2 =
      temp2 * Math.sin(phi) + v2 * Math.sin(theta2 - phi) * Math.cos(phi);
  ...
  checkWallCollisions() {
    ...
    var sortedHits = hits
      .map(hit => [hit, hit.point2.distTo(this.path.start)])
      .sort((a, b) => a[1] - b[1]);
