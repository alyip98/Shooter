...
class Projectile {
  ...
  checkCollisions(dt) {
    ...
    var nextPos = V(
      this.x + this.v * Math.cos(this.dir) * dts,
      this.y + this.v * Math.sin(this.dir) * dts
    );
    for (var i = 0; i < targets.length; i++) {
      ...
      if (dd < rr * rr)
        //dist is less than sum of radii
...
class PistolProjectile extends Projectile {
  ...
  render() {
    ...
    last = last
      .mul(this.trailLength)
      .add(V(this.x, this.y).mul(1 - this.trailLength));
